# Historical datasheet — superseded 12 September 2026

**Archive only. Counts, release status and some accuracy claims below are stale or
retracted. Do not use this file as the current dataset contract.** The current
[datasheet](../DATASHEET.md) and `data/provenance/release_facts.json` take precedence.
This archive preserves the detailed audit history and caveats without silently
deleting the previous document. In particular, the 28.2% claim below is withdrawn.
